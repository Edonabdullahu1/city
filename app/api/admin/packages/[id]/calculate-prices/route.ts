...
export async function POST(
  ...
  try {
    ...
    if (pkg.flightBlockIds && Array.isArray(pkg.flightBlockIds)) {
      ...
      for (const blockGroupId of blockGroupIds) {
        ...
        if (flights.length >= 2) {
          ...
          // Outbound: flights arriving at Istanbul (SAW)
          // Return: flights departing from Istanbul (SAW)
          const destinationAirportCode = 'SAW'; // Istanbul Sabiha Gökçen
    ...
    for (const flightBlock of flightBlocks) {
      ...
      const flightPricePerPerson = flightBlock.outbound.pricePerSeat / 100; // Convert from cents to euros
